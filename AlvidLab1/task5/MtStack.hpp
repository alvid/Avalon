...
private:
    ...
    // Для преподавателя: я специально оставил обычный мьютекс, так как устройство стека таково, что 
    // его практически невозможно использовать иначе, кроме как отдавая потоку в эксклюзивное владение.
    // Да, правда, конструктор копирования и оператор присваивания могут разрешить одновременное использование 
    // присваиваемого стека с операцией взятия размера, или проверки на пустоту.. Но возникает вопрос - раз 
    // операция копирования стека является такой важной, что мы задумываемся о ее распараллеливании, то нужен ли
    // нам вообще стек?! Я затруднился придумать подходящее использование такого странного объекта..
};
